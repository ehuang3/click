// -*- c-basic-offset: 4 -*-
#ifndef CLICK_MODELS_MULTIC_HH
#define CLICK_MODELS_MULTIC_HH
...
MultiC([I<keywords> TCPCOLLECTOR, RAW_TIMESTAMP, MIN_SCALE])
...
Calculates capacity information using equally-spaced mode gaps.  MultiC works
...
element via the TCPCOLLECTOR keyword, MultiC will write capacity information
...
MultiC has either zero or one inputs, and the same number of outputs.
...
Boolean.  If true, then input packet timestamps are used raw (MultiC will not
calculate interarrivals).  Default is false.

=item MIN_SCALE

Real number.  The initial scale to use in the MultiC algorithm.  Default is 10
microseconds.
...
Only available if MultiC has an input.

=h ack_capacities read-only

Returns capacities inferred from the current set of packet interarrivals,
assuming that these packet interarrivals are acks.  Only available if MultiC
has an input.
...
F<tcpinfo.xml> containing MultiC capacity information for every significant
flow in the trace.
...
information about one direction of a significant flow.  The MultiC element
...
Finally, you can use MultiC and FromIPSummaryDump to figure out capacity
...
class MultiC : public Element {
public:

    MultiC();
    ~MultiC();

    const char *class_name() const	{ return "MultiC"; }
    ...
    enum MultiCType { MQ_DATA, MQ_ACK };
    struct Capacity {
	double scale;
	double ntt;
	...
	Capacity(MultiCType, double scale_, double ntt_);
    };
    void run(MultiCType, Vector<double> &interarrivals /* sorted on return */,
    ...
    // tunable constants; see MultiC::MultiC for default values
  ...
  private:
    ...
    double modes2ntt(MultiCType, const Histogram &, const Vector<int> &modes) const;
    double adjust_max_scale(MultiCType, const double *begin, const double *end, double tallest_mode_min_scale) const;
    void create_capacities(MultiCType, const double *begin, const double *end, Vector<Capacity> &) const;
...
inline double
MultiC::Histogram::prob(int bin) const
{
    return _count[bin] / _nitems;
}

inline double
MultiC::Histogram::kde_prob(int bin) const
{
    return _count[bin] / (_nitems * _kde_width);
}

inline double
MultiC::Histogram::kde_sig_prob(int bin) const
{
